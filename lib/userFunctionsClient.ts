...
import type { JobFunction, User } from "@/lib/types";

export type UpdateUserFunctionsResult = { ok: true; user: User } | { ok: false; error: string };

export async function updateUserFunctions(userId: string, functions: JobFunction[]): Promise<UpdateUserFunctionsResult> {
  const result = await adminRequest<User>(`/users/${userId}/functions`, adminJsonInit("PATCH", { functions }), "Could not update job functions.");
